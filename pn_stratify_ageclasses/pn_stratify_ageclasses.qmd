---
title: Stratifying an SEIR model by age group using AlgebraicPetri.jl
date: 2024-07-22
...
We create a labelled Petri net of the SEIR model using the above transitions.
...
The age model already has `:infection`, but not `:exposure` or `:recovery`, so we add these in.
...
We rename the transitions with their group, to avoid the use of the same name in the transitions for different age groups.
...
Note that for the transmission terms, the first index refers to the susceptible group, and the second to the infected group.
...
The transitions may be easier to inspect graphically if we represent the model as a bilayer network.
...
To run the model, we need to choose specific group sizes and parameter values. Note that the ordering of the states and parameters is not lexographic.
